...
| 24 | Статусы оплаты по инвойсам и напоминания об оплате | Отображение статусов оплаты инвойсов (paid, unpaid, overdue и т.д.), механизм напоминаний клиентам об оплате. Требует backend (статусы, логика напоминаний) + UI в Back Office. |
...
| 3 | Проработать и доработать схему платных доставок | Пересмотреть текущую логику платных доставок: ценообразование, тарифы, отображение стоимости. Затрагивает Backend (расчёт, модели), Back Office (настройка тарифов), Supply Now (отображение клиенту). |
